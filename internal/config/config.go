...
func initServerConfig() (*ServerConfig, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error when converting server read timeout: '%s'. detail: %v", readTimeoutStr, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("error when converting server write timeout: '%s'. detail: %v", writeTimeoutStr, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("error when converting server max header bytes: '%s'. detail: %v", maxHeaderBytesStr, err)
	}
